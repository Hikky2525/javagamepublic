...
public class Map {
    ...
    private Parts parts = Parts.getInstance();
    // マップ
    private char[][] map;
    ...
    public void draw(Graphics g, int offsetX, int offsetY) {
        ...
        int lastTileX = firstTileX + pixelsToTiles(Game.WIDTH) + 1;
        // 描画範囲がマップの大きさより大きくならないように調整
        lastTileX = Math.min(lastTileX, col);

        int firstTileY = pixelsToTiles(-offsetY);
        int lastTileY = firstTileY + pixelsToTiles(Game.HEIGHT) + 1;
        // 描画範囲がマップの大きさより大きくならないように調整
    ...
    private void loadImage() {
    	if(cyuto ==0){
	        ImageIcon icon = new ImageIcon(getClass().getResource("image/block_kusa.gif"));
                ...
                for (int j = 0; j < col; j++) {
                    ...
                    switch (map[i][j]) {
                        case 'o':  // コイン
                            ...
                            break;
                        case 'm':  // コイン
                            sprites.add(new Mcoin(tilesToPixels(j), tilesToPixels(i), parts.getMcoin(), this));
                            break;
                        case 'k':  // 栗ボー
                            sprites.add(new Kuribo(tilesToPixels(j), tilesToPixels(i), "kuribo.gif", this,128,128));
                            break;
                        case 'w':  // 栗ボー
                            sprites.add(new Worp(tilesToPixels(j), tilesToPixels(i), "worpsaki.gif", this,16,16));
                            break;
                        case 'q':  // 栗ボー
                            sprites.add(new Obuje(tilesToPixels(j), tilesToPixels(i), "worpsaki.gif", this));
                            break;
                    }
                }
            }
