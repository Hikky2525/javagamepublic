...
public class Ranking_Felica {
	...
	final int READ_BLOCK_NUM = 0x01;   //ブロック番号はカクチェックポイントでわけている
	...
	public byte[] read() {
		...
		if (isOpened()==false) return data; // ポート接続されていなければ終了

		// カードの認識(ポインタを取得)
		Pointer felica = CLibrary.INSTANCE.felica_polling(pasori,SYSTEM_CODE,(byte)0,(byte)0);

		if (felica==Pointer.NULL) return data; // カードを認識できなければ終了



		int error=CLibrary.INSTANCE.felica_read_without_encryption02(felica, SERVICE_CODE,0,(byte) READ_BLOCK_NUM,data);

		System.out.println("戻り値" + error);
	...
	public String write() {
		...
		int error1=CLibrary.INSTANCE.felica_write_without_encryption(felica, SERVICE_CODE,(byte) READ_BLOCK_NUM,data);
	...
	public void delete5(){
		if (isOpened()==false){System.exit(1);}; // ポート接続されていなければ終了
		// カードの認識(ポインタを取得)
		Pointer felica = CLibrary.INSTANCE.felica_polling(pasori,SYSTEM_CODE,(byte)0,(byte)0);
		int[] writeBlock = new int[5];
		writeBlock[0] = 0x02;
		writeBlock[1] = 0x03;
		writeBlock[2] = 0x04;
		writeBlock[3] = 0x05;
		...
		for( int i = 0;i < writeBlock.length;i++ ){
			data[0] = (byte)5;
			data[1] = (byte)0;
			CLibrary.INSTANCE.felica_write_without_encryption(felica, SERVICE_CODE,(byte) writeBlock[i],data );
		}
	}
	...
	public int read_p(int block) {
		...
		// カードの認識(ポインタを取得)
		Pointer felica = CLibrary.INSTANCE.felica_polling(pasori, SYSTEM_CODE,
				(byte) 0, (byte) 0);
		...
		CLibrary.INSTANCE.felica_read_without_encryption02(felica,
				SERVICE_CODE, 0, (byte) block, data);
